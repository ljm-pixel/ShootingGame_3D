...
public class ExcelTool
{
    ...
    // 生成ExcelData菜单
    [MenuItem("GameTools/GenerateExcelData")]
    private static void GenerateExcelInfo()
    {
        // 创建文件夹，记在指定路径中的所有Excel文件 用于生成对应的3个文件
    ...
    private static void GenerateExcelDataClass(DataTable table)
    {
        ...
        //判断路径是否存在 没有的话 就创建文件夹GenerateExcelDataClass
    ...
    private static void GenerateExcelBinary(DataTable table)
    {
        ...
        //获取是否加密
        bool isEncryption = IsJM(table);

        //创建一个2进制文件进行写入
        using (FileStream fs = new FileStream(BinaryDataMgr.DATA_BINARY_PATH + table.TableName + ".long", FileMode.OpenOrCreate, FileAccess.Write))
        {
            //存储具体的excel对应的2进制信息
            //1.先要存储我们需要写多少行的数据 方便我们读取
            //- BEGIN_INDEX的原因是因为 前面 BEGIN_INDEX 行是配置规则 并不是我们需要记录的数据内容
            ...
            //存储是否加密的信息
            fs.Write(BitConverter.GetBytes(isEncryption), 0, 1);
            ...
            for (int i = BEGIN_INDEX; i < table.Rows.Count; i++)
            {
                ...
                for (int j = 0; j < table.Columns.Count; j++)
                {
                    switch (rowType[j].ToString())
                    {
                        case "int":
                            //加密
                            bytes = Xor(BitConverter.GetBytes(int.Parse(row[j].ToString())), isEncryption);
                            fs.Write(bytes, 0, 4);
                            break;
                        case "float":
                            //加密
                            bytes = Xor(BitConverter.GetBytes(float.Parse(row[j].ToString())), isEncryption);
                            fs.Write(bytes, 0, 4);
                            break;
                        case "bool":
                            //加密
                            bytes = Xor(BitConverter.GetBytes(bool.Parse(row[j].ToString())), isEncryption);
                            fs.Write(bytes, 0, 1);
                            break;
                        case "string":
                            bytes = Encoding.UTF8.GetBytes(row[j].ToString());
                            //写入字符串字节数组的长度
                            fs.Write(Xor(BitConverter.GetBytes(bytes.Length), isEncryption), 0, 4);
                            //加密字节数组
    ...
    private static int GetKeyIndex(DataTable table)
    {
        ...
        for (int i = 0; i < table.Columns.Count; i++)
        {
            if (row[i].ToString() == "key" || row[i].ToString() == "KeyJM")
                return i;
        }
        return 0;
    }
    ...
    /// <param name="table"></param>
    /// <returns></returns>
    private static bool IsJM(DataTable table)
    {
        DataRow row = table.Rows[3];
        for (int i = 0; i < table.Columns.Count; i++)
        {
            if (row[i].ToString() == "KeyJM")
                return true;
        }
        return false;
    }
